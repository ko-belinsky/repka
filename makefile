...
# Определяем директорию, в которой лежит Makefile
...
install: detect_de
	...
	@install -m 755 "$(THIS_DIR)create-shortcut.sh" "$(BIN_DIR)/create-shortcut.sh"
	...
	@if [ -n "$$(pgrep -x gnome-shell)" ]; then \
		...
		cp -f "$(THIS_DIR)create-shortcut.desktop" "$(KDE_SERVICES_DIR)/" 2>/dev/null || true; \
